...
def configure_fsdp_model(accelerator: Accelerator, model: DeepWorldQW) -> None:
...
def log_param_summary(accelerator: Accelerator, model: DeepWorldQW) -> None:
...
def save_config_snapshot(config: DeepWorldQWConfig, output_dir: Path, accelerator: Accelerator) -> None:
...
def open_metrics_log(output_dir: Path, accelerator: Accelerator):
	...
	return open_rank0_jsonl_log(output_dir, accelerator.is_main_process)


def build_lr_scheduler(optimizer: AdamW, config: DeepWorldQWConfig, total_training_steps: int):
...
def build_optimizer(model: DeepWorldQW, config: DeepWorldQWConfig) -> AdamW:
...
def main() -> None:
	...
	set_preliminaries()
	args = parse_args()
	config = load_qw_config(args.config)
	...
	metrics_log = open_metrics_log(output_dir, accelerator)
